...
public final class DOr implements DTree {
    ...
    public String prettyPrint() {
        return "(" + children.stream().map(DTree::prettyPrint).collect(Collectors.joining(" v ")) + ")";
    }

    @Override
    public BoolExpr toZ3(Context context) {
        return context.mkOr(children.stream()
    ...
    public Optional<DTree> prepare() {
        return Optional.of(new DOr(children.stream()
                .map(DTree::prepare)
