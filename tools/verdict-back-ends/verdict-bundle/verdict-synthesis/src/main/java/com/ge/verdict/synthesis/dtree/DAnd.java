...
public final class DAnd implements DTree {
    ...
    public String prettyPrint() {
        return "(" + children.stream().map(DTree::prettyPrint).collect(Collectors.joining(" ^ ")) + ")";
    }

    @Override
    public BoolExpr toZ3(Context context) {
        return context.mkAnd(children.stream()
    ...
    public Formula toLogicNG(FormulaFactory factory) {
        return factory.and(
                children.stream().map(child -> child.toLogicNG(factory)).collect(Collectors.toList()));
    }

    @Override
    public Optional<DTree> prepare() {
        return Optional.of(new DAnd(children.stream()
                .map(DTree::prepare)
