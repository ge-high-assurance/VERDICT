...
public class DTreeConstructor {
	...
	private Optional<DTree> constructInternal(ADTree adtree) {
		if (adtree instanceof Attack) {
			...
			// Transpose and/or
			return Optional.of(
					new DOr(adand.children().stream()
							.map(this::constructInternal)
							.flatMap(elem -> elem.isPresent()
									? Stream.of(elem.get())
									: Stream.empty())
	...
	private DTree constructDefenseTree(Defense defense) {
		return new DOr(defense.getDefenseDnf().stream().map(
				term -> new DAnd(term.stream()
						.map(leaf -> {
							String system = defense.getAttack().getSystem().getName();
							String attack = defense.getAttack().getName();
							String defenseProp = leaf.left;
											return DLeaf.createIfNeeded(system, defenseProp, attack,
													costModel.cost(defenseProp, system, dal));
						})
						.collect(Collectors.toList())))
				.collect(Collectors.toList()));
	}
