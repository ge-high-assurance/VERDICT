...
@XmlType(name = "BlameAssignment", propOrder = { "violatedProperties" })
@XmlRootElement
public class BlameAssignment {
	 ...
	 * for the violatedProperties property.
	 *
	 * <p>
	...
	public BlameAssignment compute_blame_assignment(File kind2ResultFile,
			HashMap<String, HashSet<String>> intrumented_cmp_link, boolean component_level)
	...
	public BlameAssignment blame_assignment(Vector<VerdictProperty> verdict_property,
			HashMap<String, HashSet<String>> intrumented_cmp_link, boolean component_level) {
...
//                            attack.setAttackDescription(attack.getAttackDescription());
					...
					for (Link link : mina.getLinks()) {
						...
						if (link.isCompromised()) {
							...
							List<Attack> attacks = applicableAttack(link_to_port(link.getLinkID()),
									intrumented_cmp_link);
//                            attack.setAttackDescription(attack.getAttackDescription());
							//
							// violated_property.setApplicableThreat(attack);
							for (Attack attack_found : attacks) {
