...
public class WzrdDashboard extends ApplicationWindow {
    private List<SystemTypeImpl> systems =
            new ArrayList<SystemTypeImpl>(); // stores all the SytemTypeImpl instances in the model
    ...
    public void run() {
        setShellStyle(SWT.TITLE
                ...
                | SWT.BORDER
                | SWT.MAX
                | SWT.RESIZE); // setting properties of the dashboard shell
        setBlockOnOpen(true);
        open();
    }
                    ...
                    for (int j = 0; j < systems.size(); j++) {
                        if (systems.get(j).getFullName().equals(aButton.getText())) {
                            ...
                            StatementEditor editor =
                                    new StatementEditor(systems.get(j), fileModel, shell, sourceRect, "dashboard");
                            ...
                            if (!aButton.isDisposed()) {
                                ...
                                if (checkIfSystem(systems.get(j))) {
                                    aButton.setBackground(new Color(
                                            ...
                                            145,
                                            169,
                                            216)); // system with a corresponding .impl in invoking
                                    // .aadl
                                } else {
                                    switch (colorTag) {
                                        case 0:
                                            aButton.setBackground(new Color(
                                                    ...
                                                    158,
                                                    196,
                                                    155)); // component with cyber-relation defined
                                            break;
                                        case 1:
                                            aButton.setBackground(new Color(
                                                    Display.getCurrent(),
                                                    247,
                                                    135,
                                                    135)); // a spare legend, currently not used
                                            break;
                                        case 2:
                                            aButton.setBackground(new Color(
                                                    Display.getCurrent(),
                                                    242,
                                                    174,
                                                    39)); // component with no cyber-relation defined
                                            break;
                                    }
                                }
                            }
        ...
        for (int k = 0; k < objs.size(); k++) {
            if (objs.get(k) instanceof DefaultAnnexSubclauseImpl) {
                ...
                Verdict vd = ((VerdictContractSubclause)
                                ((DefaultAnnexSubclauseImpl) objs.get(k)).getParsedAnnexSubclause())
                        .getContract();
    ...
    // check if component corr. to selected button is "top-level" (i.e. invoking .aadl contains a corresponding .impl
    // element)
