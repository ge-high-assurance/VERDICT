...
public class Agree2Vdm {
    ...
    public Model execute(File inputDir) {
        ...
        // using preprocessAadlFiles method in this class, to get objects in the aadl files in the directory along with
        // the imported aadl files
                        ...
                        for (SpecStatement specStatement : specStatements) {
                            if (specStatement instanceof EqStatement) {
                                ...
                                ContractItem contractItem =
                                        translateGuaranteeStatement(guaranteeStmt, dataTypeDecl, nodeDecl, model);
                                ...
                                ContractItem contractItem =
                                        translateAssumeStatement(assumeStmt, dataTypeDecl, nodeDecl, model);
                                contractSpec.getAssume().add(contractItem);
                            } else {
                                if (!(specStatement instanceof ConstStatementImpl)) {
                                    System.out.println("Element not recognizable"
                        ...
                        for (SpecStatement specStatement : specStatements) {
                            if (specStatement instanceof ConstStatementImpl) {
                                ...
                                ConstantDeclaration constDecl =
                                        translateConstStatementImpl(constStmtImpl, dataTypeDecl, nodeDecl, model);
                                ...
                                nodeBody =
                                        translateEqStatementImpl(eqStmtImpl, dataTypeDecl, nodeDecl, model, nodeBody);
                                ...
                                NodeEquation nodeEquation =
                                        translateAssignStatementImpl(assignStmtImpl, dataTypeDecl, nodeDecl, model);
                                ...
                                Expression assertion =
                                        translateAssertStatementImpl(assertStmtImpl, dataTypeDecl, nodeDecl, model);
    ...
    private NodeEquation translateAssignStatementImpl(
            AssignStatementImpl assignStmtImpl, HashSet<String> dataTypeDecl, HashSet<String> nodeDecl, Model model) {
        ...
        vdmNodeEquation.setRhs(
                getVdmExpressionFromAgreeExpression(assignStmtImpl.getExpr(), dataTypeDecl, nodeDecl, model));
        return vdmNodeEquation;
    }

    private Expression translateAssertStatementImpl(
            AssertStatementImpl assertStmtImpl, HashSet<String> dataTypeDecl, HashSet<String> nodeDecl, Model model) {
    ...
    private NodeBody translateEqStatementImpl(
        ...
        // get the RHS i.e.expr of the agree NodeEq and set it as the vdm node equation's RHS
        vdmNodeEquation.setRhs(
                getVdmExpressionFromAgreeExpression(eqStmtImpl.getExpr(), dataTypeDecl, nodeDecl, model));
        nodebody.getEquation().add(vdmNodeEquation);
        return nodebody;
    }

    private ConstantDeclaration translateConstStatementImpl(
            ConstStatementImpl constStmtImpl, HashSet<String> dataTypeDecl, HashSet<String> nodeDecl, Model model) {
        ...
        vdmConstDeclaration.setDefinition(
                getVdmExpressionFromAgreeExpression(constStmtImpl.getExpr(), dataTypeDecl, nodeDecl, model));
    ...
    private ContractItem translateAssumeStatement(
            AssumeStatement assumeStmt, HashSet<String> dataTypeDecl, HashSet<String> nodeDecl, Model model) {
    ...
    private SymbolDefinition translateEqStatement(
            EqStatement eqStmt, Model model, HashSet<String> dataTypeDecl, HashSet<String> nodeDecl) {
        ...
        if (type instanceof DoubleDotRef) {
            ...
            if (ddrefType.getElm() instanceof DataImplementation) { // if it is a AADL data implementation definition
                ...
                resolveAADLDataImplementationType(dataImplementationImpl, dataTypeDecl, model);
                    ...
                    instanceof org.osate.aadl2.DataType) { // if it is a AADL data implementation definition
                ...
                System.out.println("Unresolved data type " + ddrefType.getElm().getName()
    ...
    public verdict.vdm.vdm_data.DataType getVdmTypeFromAgreeType(Type type, HashSet<String> dataTypeDecl, Model model) {
        verdict.vdm.vdm_data.DataType dtype = new verdict.vdm.vdm_data.DataType();
        // need to parse the type of the variable and should map to appropriate DataType value (plainType, subrangeType,
        // arrayType, tupleType, enumType, recordType, userDefinedType) of the symbol
        if (type instanceof PrimType) {
            PrimType primType = (PrimType) type;
            verdict.vdm.vdm_data.PlainType plaintype = verdict.vdm.vdm_data.PlainType.fromValue(primType.getName());
            dtype.setPlainType(plaintype);
        } else {
            // set just name of the type - but make sure you add the type declaration to the vdm model if not already
            // added
            translateAgreeDataTypeToVdmDataType(
                    type, dataTypeDecl, model); // this call is mainly to define the type if not already defined
    ...
    private ContractItem translateGuaranteeStatement(
            GuaranteeStatement guaranteeStmt, HashSet<String> dataTypeDecl, HashSet<String> nodeDecl, Model model) {
        ContractItem contractItem = new ContractItem();
    ...
    private void addNodeDefToTypeDeclarations(
        ...
        for (NodeStmt agreeNodeStmt : agreeNodeStmts) {
            if (agreeNodeStmt instanceof NodeEqImpl) {
                ...
                // get the RHS i.e.expr of the agree NodeEq and set it as the vdm node equation's RHS
                vdmNodeEquation.setRhs(
                        getVdmExpressionFromAgreeExpression(agreeNodeEq.getExpr(), dataTypeDecl, nodeDecl, model));
    ...
    private Expression getVdmExpressionFromAgreeExpression(
        ...
        if (agreeExpr instanceof IfThenElseExpr) {
            ...
            Expression thenBranchExpr =
                    getVdmExpressionFromAgreeExpression(ifexpr.getB(), dataTypeDecl, nodeDecl, model);
            ifThenElseVal.setThenBranch(thenBranchExpr);
            Expression elseBranchExpr =
                    getVdmExpressionFromAgreeExpression(ifexpr.getC(), dataTypeDecl, nodeDecl, model);
            ...
            if (ddref.getElm() instanceof NodeDef) {
                ...
                for (Expr callExprArg : callExprArgs) {
                    Expression argExpr =
                            getVdmExpressionFromAgreeExpression(callExprArg, dataTypeDecl, nodeDecl, model);
            ...
            if (nmExpr.getElm() instanceof Arg) {
                ...
                if (!dataTypeDecl.contains(nmElmConstStatementName)) {
                    ...
                    vdmConstDeclaration.setDefinition(getVdmExpressionFromAgreeExpression(
                            nmElmConstStatement.getExpr(), dataTypeDecl, nodeDecl, model));
                    vdmConstDeclaration.setDataType(
                            getVdmTypeFromAgreeType(nmElmConstStatement.getType(), dataTypeDecl, model));
                    LustreProgram lustreProgram = model.getDataflowCode();
            ...
            if (selExpr.getField() == null) {
                ...
                if (selExpr.getTarget() != null) { // target can be NamedElmExpr or a SelectionExpr
                    vdmRecordProj.setRecordReference(
                            getVdmExpressionFromAgreeExpression(selExpr.getTarget(), dataTypeDecl, nodeDecl, model));
                }
            ...
            Expression leftOperand =
                    getVdmExpressionFromAgreeExpression(binExpr.getLeft(), dataTypeDecl, nodeDecl, model);
            binoper.setLhsOperand(leftOperand);
            // set right operand
            Expression rightOperand =
                    getVdmExpressionFromAgreeExpression(binExpr.getRight(), dataTypeDecl, nodeDecl, model);
            ...
            Expression singleOperand =
                    getVdmExpressionFromAgreeExpression(unExpr.getExpr(), dataTypeDecl, nodeDecl, model);
            ...
            // check if elm is DataImplementationImpl or DataTypeImpl -- if yes add definition to type declarations if
            // not already present
            ...
            if (recLitExpr.getRecordType() instanceof DoubleDotRef) {
                ...
                if (recType.getElm().getName() != null) {
                    // check if elm is DataImplementationImpl -- if yes add definition to type declarations if not
                    // already present
                    ...
                    for (int ind = 0; ind < recLitArgs.size(); ind++) {
                        ...
                        fieldDef.setFieldValue(getVdmExpressionFromAgreeExpression(
            ...
            if (eventExpr.getPort() != null) {
                vdmExpr.setEvent(
                        getVdmExpressionFromAgreeExpression(eventExpr.getPort(), dataTypeDecl, nodeDecl, model));
            } else {
            ...
            vdmExpr.setToReal(
                    getVdmExpressionFromAgreeExpression(realCastExpr.getExpr(), dataTypeDecl, nodeDecl, model));
        } else {
