...
public class WzrdClosure {
    ...
    private boolean invalidReq =
            false; // boolean containing information whether an entered cyber-property is invalid/incomplete
    private boolean invalidMission =
            false; // boolean containing information whether an entered cyber-mission is invalid/incomplete
    private boolean invalidMissionFound =
            false; // boolean containing information whether any of the entered cyber-mission is invalid/incomplete
    ...
    private void parseForCorrectness() {
        for (int i = 0; i < tableContents.size(); i++) {
            if (tableContents.get(i).isNew()) {
                Injector injector = VerdictActivator.getInstance().getInjector(VerdictUiModule.INJECTOR_NAME);
    ...
    private OffsetTabPair getOffsetTab(IPath filePath) throws IOException {
        ...
        FileReader file = new FileReader(ResourcesPlugin.getWorkspace()
                .getRoot()
                .getFile(filePath)
                .getRawLocationURI()
                .toURL()
