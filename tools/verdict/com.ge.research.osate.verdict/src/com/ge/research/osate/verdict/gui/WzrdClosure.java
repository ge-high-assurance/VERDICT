...
public class WzrdClosure {
	...
	private boolean invalidReq = false; // boolean containing information whether an entered cyber-property is invalid/incomplete
	private boolean invalidMission = false; // boolean containing information whether an entered cyber-mission is invalid/incomplete
	private boolean invalidMissionFound = false; // boolean containing information whether any of the entered cyber-mission is invalid/incomplete
	...
	public WzrdClosure(SystemTypeImpl sys, List<WzrdTableRow> tableContents, List<WzrdTableRow> deleteContents,
			IPath filePath, boolean systemLevel, DrpDnLists drpdn, Composite composite, List<MissionInfo> missions,
			Shell shell) {
		...
		if (openEditor != null) {
			...
			if (response && openEditor.isDirty()) {
				MessageDialog.openError(shell, "VERDICT Wizard Launcher",
		...
		if (failedToParseRows.size() > 0) {
			...
			errStr = errStr + "While saving, those rows will be discarded by Wizard. Are you sure to continue?";
			invalidReq = !(MessageDialog.openConfirm(composite.getShell(), "Wizard: Confirmation", errStr));
		}
		if (invalidMissionFound) {
			errStr = "At least one mission does not have any cyber-requirement assigned. ";
			errStr = errStr + "While saving, those missions will be discarded by Wizard. Are you sure to continue?";
			invalidMission = !(MessageDialog.openConfirm(composite.getShell(), "Wizard: Confirmation", errStr));
		}
	...
	private boolean validate(Composite composite) {
		...
		for (Map.Entry<String, Integer> entry : counts.entrySet()) {
			if (entry.getValue() > 1) {
				MessageDialog.openError(composite.getShell(), "Duplicate ID",
	...
	private void parseForCorrectness() {
		for (int i = 0; i < tableContents.size(); i++) {
			if (tableContents.get(i).isNew()) {
				...
				try {
					parseResult = parser.parse(parser.getGrammarAccess().getAnnexSubclauseRule(),
	...
	private void generateAppendCode(Boolean hasAnnex) {
		...
		for (int i = 0; i < tableContents.size(); i++) {
			...
			if (systemLevel) {
				...
				str = addTab(tabCount + 2, str) + "condition = " + tableRow.getThirdElement() + "\n";
				str = addTab(tabCount + 2, str) + "cia = " + drpdn.CIA_ABBREV[tableRow.getFirstElement()] + "\n";
				str = addTab(tabCount + 2, str) + "severity = " + drpdn.SEVERITY[tableRow.getSecondElement()] + "\n";

				if ((tableRow.getComment() != null) && tableRow.getComment() != "") {
					str = addTab(tabCount + 2, str) + "comment = \"" + tableRow.getComment() + "\"\n";
				}
				if ((tableRow.getDescription() != null) && tableRow.getComment() != "") {
					str = addTab(tabCount + 2, str) + "description = \"" + tableRow.getDescription() + "\"\n";
				}
				...
				if ((tableRow.getExternal() != null) && tableRow.getComment() != "") {
					str = addTab(tabCount + 2, str) + "external = \"" + tableRow.getPhase() + "\"\n";
				}
				...
				if (!tableRow.getThirdElement().equals("TRUE")) {
					str = addTab(tabCount + 2, str) + "inputs = " + tableRow.getThirdElement() + "\n";
				}
				str = addTab(tabCount + 2, str) + "output = " + drpdn.outPorts[tableRow.getFirstElement()] + ":"
		...
		if (systemLevel && missions.size() > 0) {
			for (int i = 0; i < missions.size(); i++) {
				...
				if ((mission.getComment() != null) && mission.getComment() != "") {
					str = addTab(tabCount + 2, str) + "comment = \"" + mission.getComment() + "\"\n";
				}
				if ((mission.getDescription() != null) && mission.getComment() != "") {
					str = addTab(tabCount + 2, str) + "description = \"" + mission.getDescription() + "\"\n";
				}
	...
	private void appendMakeReady(Boolean hasAnnex, IPath filePath) {
		...
		for (int i = 0; i < tableContents.size(); i++) {
			...
			if (i == 0) {
				...
				if (!hasAnnex) {
					appendix = appendix + "\n";
					appendix = addTab(tabCount + 2, appendix) + tableContents.get(i).getVerdictStatement();
				} else {
					appendix = appendix + "\n";
					appendix = addTab(tabCount + 1, appendix) + tableContents.get(i).getVerdictStatement();
				}
			}
		}
		...
		if (systemLevel && (missions.size() > 0)) {
			for (int i = 0; i < missions.size(); i++) {
				...
				if (!hasAnnex) {
					appendix = appendix + "\n";
					appendix = addTab(tabCount + 2, appendix) + missions.get(i).getVerdictStatement();
				} else {
					appendix = appendix + "\n";
					appendix = addTab(tabCount + 1, appendix) + missions.get(i).getVerdictStatement();
				}
			}
		}
		...
		if (!hasAnnex) {
			...
			for (int i = 0; i < objs.size(); i++) {
				if (objs.get(i) instanceof DefaultAnnexSubclauseImpl) {
					...
					offset = offset - offTab.addOffset + 1; // 1 appear here since no semicolon if annex is present
					break;
				}
			}
		}
	...
	private OffsetTabPair getOffsetTab(IPath filePath) throws IOException {
		...
		FileReader file = new FileReader(
				ResourcesPlugin.getWorkspace().getRoot().getFile(filePath).getRawLocationURI().toURL().getPath());
		int c;
		int count = 0;
		int addOffset = 0;
		int tabCount = 0;
