            ...
            Thread createGsnThread = new Thread() {
                ...
                public void run() {
                    try {
                        ...
                        // running MBAS
                        boolean outputsGenerated = runBundleMBAS(
                                bundleJar, dockerImage, projectDir.getName(), stemProjPath, soteriaPpBin, graphVizPath);
                            ...
                            if (runBundle(
                                ...
                                if (AssuranceCaseSettingsPanel.showInTab) {
                                    ...
                                    VerdictHandlersUtils.openSvgGraphsInDir(
                                            new File(stemProjPath, "gsn").getAbsolutePath());
                                }
                            }
                        }
                ...
                if (securityFlag) {
                    if (missionFlag) { // otherwise add only provided mission Ids
                        ...
                        List<String> duplicateFreeSupportingCyberForUserProvidedMissions =
                                new ArrayList<>(new HashSet<>(supportingCyberForUserProvidedMissions));
