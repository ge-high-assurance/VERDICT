...
public class ChangeInFile {
	...
	// updates file
	protected void refreshFile(int offset, boolean needToAppend, String appendix, Boolean systemLevel)
			throws IOException {
		...
		FileReader file = new FileReader(
				ResourcesPlugin.getWorkspace().getRoot().getFile(modelPath).getRawLocationURI().toURL().getPath());
		String wrtpath = ResourcesPlugin.getWorkspace().getRoot().getFile(ipath.append("/tmp_tmp.aadl"))
				.getRawLocationURI().toURL().getPath();
		...
		file.close();
		tmpFile.close();
		String newpath = ResourcesPlugin.getWorkspace().getRoot().getFile(modelPath).getRawLocationURI()
				.toURL().getPath();
				...
				if (i <= offsets.size() - 1) {
					if (count == offsets.get(i) + 1) {
						...
						if (!missionTags.get(i)) {
							offsets.set(i, offsets.get(i) - 9 - spaceCount - tabCount); // constant 9 is for keyword "CyberReq"/"CyberRel" and a \n
							lengths.set(i, lengths.get(i) + 9 + spaceCount + tabCount); // constant 9 is for keyword "CyberReq"/"CyberRel" and a \n
						} else {
							offsets.set(i, offsets.get(i) - 11 - spaceCount - tabCount); // constant 11 is for keyword "MissionReq" and a \n
							lengths.set(i, lengths.get(i) + 11 + spaceCount + tabCount); // constant 11 is for keyword "MissionReq" and a \n
						}
						spaceCount = 0;
						tabCount = 0;
						continue;
