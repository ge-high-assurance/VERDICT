...
public class StatementEditor extends ApplicationWindow {
    ...
    public StatementEditor(
            SystemTypeImpl sys, IPath sourceFile, Shell lastShell, Rectangle sourceRect, String callingClass) {
        super(lastShell);
    ...
    protected Control createContents(Composite parent) {
            ...
            addEditMission.addSelectionListener(new SelectionAdapter() {
                ...
                public void widgetSelected(SelectionEvent event) {
                    if (tableContent.size() > 0) {
                        MissionEditor ldEditor = new MissionEditor(
                                composite.getShell(), sourceRect, tableContent, missions, drpdn, idSet);
                                ...
                                if (tableContent.get(j).getFormulaID().equals(formulaID)) {
                                    if (!ignore) {
                                        MessageDialog dialog = new MessageDialog(
                                                ...
                                                MessageDialog.ERROR,
                                                new String[] {"Yes to all", "Yes", "No", "Cancel"},
                                                0);
                                        result = dialog.open();
                                    } else {
                                        result = 1;
                                    ...
                                    switch ((String) str) {
                                        case ("Confidentiality (C)"): {
                                            rowToDelete.setSecondElement(
                                                    drpdn.findIndex(drpdn.CIA, "Confidentiality (C)"));
                                        ...
                                        case ("Availability (A)"): {
                                            rowToDelete.setSecondElement(
                                                    drpdn.findIndex(drpdn.CIA, "Availability (A)"));
            ...
            public void widgetSelected(SelectionEvent event) {
                ...
                if (!checkConsistency(tableContent, tv.getTable().getSelectionIndex())) {
                    ...
                    LogicDefinitionEditor ldEditor =
                            new LogicDefinitionEditor(composite.getShell(), drpdn, systemLevel, sourceRect);
                    if (ldEditor.isValid()) {
                        composite.setEnabled(false);
                        ldEditor.run();
                        composite.setEnabled(true);
        ...
        save.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent event) {
                WzrdClosure closing = new WzrdClosure(
                        selectedSys,
                        tableContent,
        ...
        editors[0].addListener(new ICellEditorListener() {
            ...
            public void applyEditorValue() {
                ...
                if (editorContentCol0 != (String) editors[0].getValue()) {
                    ...
                    //										"Entered ID is identical to one of the existing cyber-requirement IDs. Try a different
                    // ID.");
                    //								tv.refresh();
                    //								break breakout;
                    //							} else {
                    //								MessageDialog.openError(composite.getShell(), "VERDICT Wizard Error",
                    //										"Entered ID is identical to one of the existing cyber-relation IDs. Try a different
                    // ID.");
                    //								tv.refresh();
                    //								break breakout;
                    //							}
                    ...
                    //						MessageDialog.openError(composite.getShell(), "VERDICT Wizard Error",
                    //								"Entered ID matches the ID of an existing field of another system/component in the same
                    // .aadl file. Try a different ID.");
                    //						tv.refresh();
                    //						break breakout;
                    //					}
    ...
    private void updateMissionString(int currIndex, String str) {
        ...
        for (int i = 0; i < missions.size(); i++) {
            missionString = missionString + Integer.toString(i + 1) + ". "
                    + missions.get(i).getMissionID();
                ...
                if (currIndex > 0) {
                    if (index == currIndex) {
                        ...
                        if (index > currIndex) {
                            if (str != null) {
                                missionString = missionString
                                        + tableContent.get(index - 1).getFormulaID();
                            } else {
                                missionString =
    ...
    private SystemTypeImpl reloadSystem(SystemTypeImpl sys) {
        ...
        while (tree.hasNext()) {
            ...
            if (anObject instanceof DefaultAnnexSubclauseImpl) {
                ...
                Verdict vd = ((VerdictContractSubclause)
                                ((DefaultAnnexSubclauseImpl) anObject).getParsedAnnexSubclause())
                        .getContract();
                List<Statement> stmts = vd.getElements();
                for (int i = 0; i < stmts.size(); i++) {
                    if (stmts.get(i) instanceof CyberMissionImpl) {
