...
public class Vdm2Csv {
    ...
    public void execute(Model vdm, String stemDir, String soteriaDir, String modelName) {
        Table eventsTable =
                new Table("QualifiedName", "SanitizedQualifiedName", "PackageName", "Comp", "Event", "Probability");
        Table compSafTable = new Table(
                "QualifiedName",
        ...
        Table scnConnTable = updateConnectionsTable(
                compImpls, modelName, propToConnections, connectionAttributesMap, compToCompImpl);
        Table scnBusBindingsTable = updateBusBindingsTable(compImpls, modelName, compToCompImpl);
        Table scnCompPropsTable =
                updateCompInstTable(compImpls, modelName, propToCompInsts, compInstAttributesMap, compToCompImpl);
        // build events, compDep, compSaf, mission tables
        updateTablesWithDataFromVDM(
                vdm, modelName, eventsTable, compDepTable, compSafTable, missionTable, connectionDestToSourceMap);
    ...
    private void updateTablesWithDataFromVDM(
        ...
        processMissionReqs(
                inputVDM.getMission(), cyberReqsMap, safetyReqsMap, missionTable, modelName, connectionDestToSourceMap);
    }
        ...
        for (ComponentImpl compImpl : compImpls) {
            if (compImpl.getBlockImpl() != null) {
                ...
                for (verdict.vdm.vdm_model.Connection compConnection1 : compConnections) {
                    ...
                    // -- Create a map of connections that have a data-port as destination rather than
                    // subcomponent.data-port
                ...
                for (verdict.vdm.vdm_model.ComponentInstance compInst : compInstances) {
                    ...
                    for (GenericAttribute attr : compInstAttributesList) {
                        ...
                        // -- Get all property names associated with components --> need it for headers in components
                        // csv
    ...
    private void updateConnectionTable(
        ...
        if (source.getComponentPort() != null) {
            ...
            // remove port name from fullQualNameSrcCompPort and text before "::" and after "." to get the source
            // component name
            String compName = fullQualNameSrcCompPort.substring(
                    fullQualNameSrcCompPort.indexOf(':') + 2, fullQualNameSrcCompPort.indexOf('.'));
            ...
            String sourceInstQualName =
                    source.getSubcomponentPort().getSubcomponent().getId();
            ...
            scnConnTable.addValue(
                    srcCompPort.getSubcomponent().getSpecification().getCompCateg()); // SrcCompCategory
        ...
        if (destination.getComponentPort() != null) {
            ...
            // get the component name
            // remove port name from fullQualNameDestCompPort and text before "::" and after "." to get the destination
            // component name
            String compName = fullQualNameDestCompPort.substring(
                    fullQualNameDestCompPort.indexOf(':') + 2, fullQualNameDestCompPort.indexOf('.'));
            if (!compToCompImpl.containsKey(compName)) {
                throw new RuntimeException("Unable to find Component Implementation corresponding to " + compName);
            }
            ...
            String destInstQualName =
                    destination.getSubcomponentPort().getSubcomponent().getId();
            ...
            // get the component name
            scnConnTable.addValue(destination
                    .getSubcomponentPort()
                    .getSubcomponent()
                    .getSpecification()
                    .getName()); // DestComp
            ...
            scnConnTable.addValue(
                    destCompPort.getSubcomponent().getSpecification().getCompCateg()); // DestCompCategory
        ...
        for (String propName : propToConnections.keySet()) {
            if (synthesis && DefenseProperties.MBAA_CONN_DEFENSE_PROPERTIES_SET.contains(propName)) {
                ...
                scnConnTable.addValue("9");
                    ...
                    propName)) { // check if the connection has that property - add it to csv if it does
    ...
    // Assumption: component implementation containing the connection is same as the component implementation where the
    // actual_connection_binding property was defined
                ...
                for (verdict.vdm.vdm_model.Connection compConnection2 : compConnections) {
                    if (compConnection2.getActualConnectionBinding() != null) {
                        ...
                        scnBusBindingsTable.addValue(
                                replaceColonsWithUnderscore(compQualName)); // SanitizedQualifiedName
                        scnBusBindingsTable.addValue(
                                compQualName.substring(0, compQualName.indexOf(':'))); // PackageName
                        ...
                        String bindingSubStr = actualConnectionBinding.substring(
                                0, actualConnectionBinding.lastIndexOf(".")); // everything except the instance name
                        String implName =
                                bindingSubStr.substring(bindingSubStr.lastIndexOf(":") + 1, bindingSubStr.length());
                        if (!compImplToComp.containsKey(implName)) {
                            throw new RuntimeException(
                                    "Unable to find Component corresponding to Implementation " + implName);
                        }
                        ...
                        scnBusBindingsTable.addValue(replaceColonsWithUnderscore(
                                actualConnectionBinding)); // SanitizedActualConnectionBindingSrcBusInstName
                        scnBusBindingsTable.addValue(actualConnectionBinding.substring(
                                actualConnectionBinding.lastIndexOf('.') + 1,
                                actualConnectionBinding.length())); // ActualConnectionBindingSrcBusInst
                        ...
                        scnBusBindingsTable.addValue(
                                compImpl.getType().getName()); // ActualConnectionBindingDestConnComp
                        ...
                        scnBusBindingsTable.addValue(replaceColonsWithUnderscore(
                                compConnection2.getQualifiedName())); // SanitizedActualConnectionBindingDestConnName
    ...
    private Table updateCompInstTable(
        ...
        List<String> headers = new ArrayList<String>(Arrays.asList(
                "Scenario", "QualifiedName", "SanitizedQualifiedName", "PackageName", "Comp", "Impl", "CompInstance"));
            ...
            for (String cyberReqId : cyberReqIds) {
                ...
                if (cyberReqsMap.containsKey(cyberReqId)) {
                    ...
                    if (cyberReqCondition.getKind()
                            != null) { // if condition is an OR expression get individual sub-expressions
                        if (cyberReqCondition.getKind().toString().equalsIgnoreCase("Or")) {
                            List<CyberExpr> subCyberCondList =
                                    cyberReqCondition.getOr().getExpr();
    ...
    private void updateCyberMissionsTable(
        ...
        if (connectionDestToSourceMap.containsKey(cyberReqCondition.getPort().getName())) {
            ...
            throw new RuntimeException("Missing component instance dependency. "
                    + cyberReqCondition.getPort().getName() + " is not linked to a source port");
        }
    }
    ...
    private void updateSafetyMissionsTable(
        ...
        if (connectionDestToSourceMap.containsKey(safetyReqCondition.getPort().getName())) {
            ...
            throw new RuntimeException("Missing component instance dependency. "
                    + safetyReqCondition.getPort().getName() + " is not linked to a source port");
        }
    }

    private void processComponentTypes(
            List<ComponentType> compTypes, Table eventsTable, Table compDepTable, Table compSafTable) {
        // each ComponentType contains id, name, compCateg, List<Port>, ContractSpec, List<CyberRel>, List<SafetyRel>,
        // List<Event>
    ...
    private void updateCompSafTable(
        ...
        if (safetyRelExpr.getKind() == null) {
            ...
            updateCompSafTableWithAndExpr(
                    compSafTable, qualNameComp, packageName, compTypeName, safetyRelExpr, outputIAPort);
        } else {
        ...
        if (safetyRelExpr.getKind() == null) {
            if (portsIAs[0].equalsIgnoreCase("")) {
                if (safetyRelExpr.getPort() != null) {
                    ...
                    portsIAs[1] =
                            formatToSmall(safetyRelExpr.getFault().getHappens().toString());
                }
            } else {
                if (safetyRelExpr.getPort() != null) {
                    portsIAs[0] = portsIAs[0] + ";" + safetyRelExpr.getPort().getName();
                    portsIAs[1] = portsIAs[1] + ";"
                            + formatToSmall(safetyRelExpr.getPort().getIa().name());
                } else { // assuming it is an event if not a port
    ...
    private void updateCompDepTable(
        ...
        for (verdict.vdm.vdm_model.CyberRel cyberRel : cyberRels) {
            if (cyberRel.getInputs() != null) {
                ...
                compDepTable.addValue(
                        formatToSmall(cyberRel.getOutput().getCia().name()));
                compDepTable.capRow();
            }
        }
    }
    ...
    private void updateCompDepTable(
        ...
        if (inputCyberExpr.getKind() == null) { // expression is not an AND, OR, NOT expression
            ...
            for (CyberExpr subInpCyberExpr : subInpCyberList) {
                updateCompDepTable(
                        compDepTable, qualNameComp, packageName, compTypeName, subInpCyberExpr, outputCIAPort);
            }
        } else if (inputCyberExpr.getKind().toString().equalsIgnoreCase("And")) {
            updateCompDepTableWithAndCyberExpr(
                    compDepTable, qualNameComp, packageName, compTypeName, inputCyberExpr, outputCIAPort);
        } else {
    ...
    private void getPortsAndCIAsForMission(
            CyberExpr inputCyberExpr, String[] portsCIAs, Map<String, List<ConnectionEnd>> connectionDestToSourceMap) {
        if (inputCyberExpr.getKind() == null) {
            // get port's linked source instance
            if (connectionDestToSourceMap.containsKey(inputCyberExpr.getPort().getName())) {
                ...
                if (linkedSourcePorts.get(0).getSubcomponentPort() != null) {
                    ...
                    if (portsCIAs[0].equalsIgnoreCase("")) {
                        ...
                        portsCIAs[0] = portsCIAs[0] + ";"
                                + depCompPort.getSubcomponent().getName();
                        portsCIAs[1] =
                                portsCIAs[1] + ";" + depCompPort.getPort().getName();
                        portsCIAs[2] = portsCIAs[2] + ";"
                                + formatToSmall(
                                        inputCyberExpr.getPort().getCia().name());
                    }
                } else {
                    throw new RuntimeException("Linked Source Port has no instance information");
                }
                ...
                throw new RuntimeException("Missing component instance dependency. "
                        + inputCyberExpr.getPort().getName() + " is not linked to a source port");
            }
        } else if (inputCyberExpr.getKind().toString().equalsIgnoreCase("And")) {
            List<CyberExpr> subInpCyberList = inputCyberExpr.getAnd().getExpr();
            for (CyberExpr subInpCyberExpr : subInpCyberList) {
            ...
            if (connectionDestToSourceMap.containsKey(inputSafetyExpr.getPort().getName())) {
                ...
                if (linkedSourcePorts.get(0).getSubcomponentPort() != null) {
                    ...
                    if (portsIAs[0].equalsIgnoreCase("")) {
                        ...
                        portsIAs[0] = portsIAs[0] + ";"
                                + destCompPort.getSubcomponent().getName();
                        portsIAs[1] = portsIAs[1] + ";" + destCompPort.getPort().getName();
                        // get IA
                        portsIAs[2] = portsIAs[2] + ";"
                                + formatToSmall(
                                        inputSafetyExpr.getPort().getIa().name());
                    }
                } else {
                    throw new RuntimeException("Linked Source Port has no instance information");
                }
                ...
                throw new RuntimeException("Missing component instance dependency. "
                        + inputSafetyExpr.getPort().getName() + " is not linked to a source port");
            }
