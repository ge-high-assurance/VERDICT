...
public class SafetyCutsetsView extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		for (int i = 0; i < cutsets.size(); i++) {
			...
			String eventsText = String.join(" and ",
					cutset.getEvents().stream().map(event -> event.getComponent() + ":" + event.getEventName())
							.collect(Collectors.toList()));
			item.setText(new String[] { "# " + Integer.toString(i + 1), cutset.getLikelihood(), eventsText });
		}
