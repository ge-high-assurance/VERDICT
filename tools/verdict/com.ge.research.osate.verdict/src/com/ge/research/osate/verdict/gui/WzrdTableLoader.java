...
public class WzrdTableLoader {
    ...
    private boolean noExistingVerdictAnnex =
            true; // boolean containing information about existence of existing verdict annex in the selected
    // SystemTypeImpl in
    // invoking .aadl

    private List<WzrdTableRow> tableContent = new ArrayList<WzrdTableRow>(); // stores the extracted information
    private boolean
            systemLevel; // boolean containing information about whether selected SystemTypeImpl instance is a component
    // or a system
    private DrpDnLists
            drpdn; // contains the drop-down list contents and information/header of the available INPorts/OUTPorts
        ...
        for (int i = 0; i < objs.size(); i++) {
            if (objs.get(i) instanceof DefaultAnnexSubclauseImpl) {
                ...
                Verdict vd = ((VerdictContractSubclause)
                                ((DefaultAnnexSubclauseImpl) objs.get(i)).getParsedAnnexSubclause())
                        .getContract();
    ...
    private WzrdTableRow getTableRow(Statement stmt) {
        ...
        for (int j = 0; j < objList.size(); j++) {
            ...
            if (objInst instanceof CyberRelOutputLogic) {
                ...
                ISerializer serializer = VerdictActivator.getInstance()
    ...
    private WzrdTableRow getTableRow(CyberReq stmt) {
        ...
        tableRow.setSecondElement(
                drpdn.findIndex(drpdn.SEVERITY, stmt.getSeverity().getLiteral()));
