...
public class LogicDefinitionEditor extends ApplicationWindow {
    ...
    public LogicDefinitionEditor(Shell shell, DrpDnLists list, boolean sysLevel, Rectangle sourceRect) {
        super(shell);
        lastShell = shell;
        drpdn = list;
    ...
    protected Control createContents(Composite parent) {
        ...
        // defines the activities to perform on clicking the button
        lOr.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent event) {
        ...
        // defines the activities to perform on clicking the button
        lNot.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent event) {
                formulaBuffer.add("!");
                label.setText(startLine + buffer2String(formulaBuffer));
        ...
        // defines the activities to perform on clicking the button
        comboLeft.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent event) {
                if (comboLeft.getSelectionIndex() > 0) {
                    if (systemLevel) {
                        tmpBuffer.add(drpdn.outPortsWithNull[max(comboLeft.getSelectionIndex(), 0)]);
                    } else {
                        tmpBuffer.add(drpdn.inPortsWithNull[max(comboLeft.getSelectionIndex(), 0)]);
                    }
        ...
        // defines the activities to perform on clicking the button
        comboRight.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent event) {
                if (comboRight.getSelectionIndex() > 0) {
                    tmpBuffer.add(":" + drpdn.CIA_ABBEV_WITH_NULL[max(comboRight.getSelectionIndex(), 0)]);
        ...
        composite.getShell().addListener(SWT.Close, event -> {
            if (exportDefinition) {
