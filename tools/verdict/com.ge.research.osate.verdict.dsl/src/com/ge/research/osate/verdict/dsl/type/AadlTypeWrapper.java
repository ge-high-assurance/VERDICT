        ...
        if (type instanceof EnumerationType) {
            ...
            isValue = value -> value != null
                    && ((EnumerationType) type)
                            .getOwnedLiterals().stream().anyMatch(literal -> value.equals(literal.getName()));
            // Suggest all enumeration literals
            valueSuggestions = ((EnumerationType) type)
            ...
            fields = ((RecordType) type)
                    .getOwnedFields().stream()
                            .map(field -> new VerdictFieldImpl(
                                    field.getName(), new AadlTypeWrapper(field.getName(), field.getPropertyType())))
                            .collect(Collectors.toList());
