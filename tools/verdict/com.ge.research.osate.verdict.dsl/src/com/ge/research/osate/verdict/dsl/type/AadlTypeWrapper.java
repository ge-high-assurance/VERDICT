...
/**
 * A type that is loaded from the type of a property,
 * wrapping an existing PropertyType. Used in threat models.
 */
public class AadlTypeWrapper implements VerdictType {
	...
	/**
	 * Converts a possibly-exception-raising function call into one
	 * that returns an option that is non-present upon an exception.
	 *
	 * This isn't super-useful for what we're doing here, but it is
	 * cool.
	 *
	 * @param <I> input type
	 * @param <O> output type
	 ...
	 *         or empty if the function threw an exception
	 */
	...
	public AadlTypeWrapper(String name, PropertyType type) {
		...
		if (type instanceof EnumerationType) {
			...
			isValue = value -> value != null && ((EnumerationType) type).getOwnedLiterals().stream()
					.anyMatch(literal -> value.equals(literal.getName()));
			// Suggest all enumeration literals
			valueSuggestions = ((EnumerationType) type).getOwnedLiterals().stream().map(enumVal -> enumVal.getName())
			...
			isValue = value -> value != null && ((value.startsWith("\"") && value.endsWith("\""))
			...
			isValue = value -> {
				...
				if (!result.isPresent() || (range.isPresent() && (range.get().min > result.get() || range.get().max < result.get()))) {
					return false;
				}
				return true;
			};
			...
			isValue = value -> {
				...
				// Value must be in range
				if (!result.isPresent()
						|| (range.isPresent() && (range.get().min > result.get() || range.get().max < result.get()))) {
					return false;
				}
				return true;
			};
			...
			fields = ((RecordType) type).getOwnedFields().stream()
					.map(field -> new VerdictFieldImpl(field.getName(),
							new AadlTypeWrapper(field.getName(), field.getPropertyType())))
					.collect(Collectors.toList());
	...
	public boolean equals(Object other) {
		if (other instanceof VerdictType) {
			...
			return isList() == otherType.isList() && getFullName().equals(otherType.getFullName())
