...
public class VerdictAnnexParser implements AnnexParser {
    ...
    public AnnexLibrary parseAnnexLibrary(
            String annexName, String source, String filename, int line, int column, ParseErrorReporter errReporter) {
        return (VerdictContractLibrary) AnnexParseUtil.parse(
                parser, source, getGrammarAccess().getAnnexLibraryRule(), filename, line, column, errReporter);
    }

    @Override
    public AnnexSubclause parseAnnexSubclause(
            String annexName, String source, String filename, int line, int column, ParseErrorReporter errReporter) {
        return (VerdictContractSubclause) AnnexParseUtil.parse(
                parser, source, getGrammarAccess().getAnnexSubclauseRule(), filename, line, column, errReporter);
    }
}
