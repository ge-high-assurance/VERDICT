...
public class VerdictProposalProvider extends AbstractVerdictProposalProvider {
	 ...
	 * @param acceptor
	 */
	private void acceptOrderedProposals(String[] proposals, ContentAssistContext context,
		...
		for (int i = 0; i < proposals.length; i++) {
			...
			acceptor.accept(createCompletionProposal(proposals[i], null, null, base + proposals.length - i,
					context.getPrefix(), context));
		}
	}

	@Override
	public void complete_LPort(EObject model, RuleCall ruleCall, ContentAssistContext context,
			ICompletionProposalAcceptor acceptor) {
	...
	@Override
	public void complete_CIA(EObject model, RuleCall ruleCall, ContentAssistContext context,
			ICompletionProposalAcceptor acceptor) {
	...
	@Override
	public void completeCyberMissionBlock_CyberReqs(EObject model, Assignment assignment, ContentAssistContext context,
			ICompletionProposalAcceptor acceptor) {
	...
	@Override
	public void completeIntro_Type(EObject model, Assignment assignment, ContentAssistContext context,
			ICompletionProposalAcceptor acceptor) {
	...
	@Override
	public void complete_Var(EObject model, RuleCall ruleCall, ContentAssistContext context,
			ICompletionProposalAcceptor acceptor) {

		// Completes with variables that are in scope (introduced by intro clauses, i.e. forall and exists)

		EObject scopeParent = ThreatModelUtil.getContainerForClasses(model,
				ThreatModelUtil.VAR_FIELD_SCOPE_PARENT_CLASSES);

		if (scopeParent != null && scopeParent.eResource() != null
		...
		if (model != null) {
			...
			if (container instanceof ThreatEqualContains) {
				...
				if (equalContains.isEqual() && equalContains.getLeft() != null
					...
					ThreatModelUtil.FieldTypeResult res = ThreatModelUtil.getVarType(equalContains.getLeft(),
							indexProvider);
					if (res.type.isPresent()) {
						suggestions = res.type.get().getValueSuggestions();
					}
				}
				if (equalContains.isEqual() && equalContains.getRight() != null
					...
					ThreatModelUtil.FieldTypeResult res = ThreatModelUtil.getVarType(equalContains.getRight(),
							indexProvider);
					if (res.type.isPresent()) {
						suggestions = res.type.get().getValueSuggestions();
					}
	...
	@Override
	public void complete_ThreatEqualContains(EObject model, RuleCall ruleCall, ContentAssistContext context,
			ICompletionProposalAcceptor acceptor) {
	...
	@Override
	public void completeVar_Ids(EObject model, Assignment assignment, ContentAssistContext context,
			ICompletionProposalAcceptor acceptor) {
	...
	@Override
	public void completeThreatDefenseBlock_Threats(EObject model, Assignment assignment, ContentAssistContext context,
			ICompletionProposalAcceptor acceptor) {
