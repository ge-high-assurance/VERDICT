...
/**
 * Custom variable resolution for templates. Required because
 * Osate does not support annex templates.
 *
 * Resolves template variables named 'VerdictTemplateVar'
 * and uses the first parameter to determine direction.
 *
...
public class VerdictTemplateVariableResolver extends AbstractTemplateVariableResolver {
    ...
    public List<String> resolveValues(TemplateVariable variable, XtextTemplateContext xtextTemplateContext) {
        ...
        if (variable.getType().equals(TYPE)
                && !variable.getVariableType().getParams().isEmpty()) {
